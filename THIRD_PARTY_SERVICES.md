...
| **Anthropic Claude Code** | Official CLI installed and run on-device to chat with Claude | You, via the official `claude auth login` browser flow | The Claude Code CLI process talks directly to Anthropic (or your configured provider) | Inside the Claude Code CLI's own credential store, inside the on-device Debian rootfs — not read or copied by AndCode | No | [Anthropic Consumer/Commercial Terms](https://www.anthropic.com/legal) and Claude Code's own terms | **Sign out** in **Settings → Agents → Claude Code**, or delete the local runtime | Anthropic's official CLI; AndCode is not affiliated with or endorsed by Anthropic |
...
| **Debian** (`registry-1.docker.io`) | Bookworm slim OCI image used as the base rootfs for the on-device runtime | N/A (unauthenticated download) | The app downloads the official `debian:12-bookworm-slim` OCI blob over HTTPS | N/A | No | [Debian's licenses](https://www.debian.org/legal/) | Delete the local runtime | Independent open-source project |
| **Debian apt mirrors** (`deb.debian.org`, `security.debian.org`) | Bookworm packages installed into the shared sandbox and the Antigravity rootfs | N/A (unauthenticated `apt` mirrors) | `apt` inside the Debian rootfs fetches packages over the configured mirrors | N/A | No | [Debian's licenses](https://www.debian.org/legal/) (per-package) | Delete the local runtime | Independent open-source project |
...
1. **A provider API key you type into AndCode's own UI, used with the local (on-device) OpenCode
   ...
   *and* — via `LocalProviderCredentialStore.syncToRuntime()` — written into
   `root/.local/share/opencode/auth.json` inside the on-device Debian rootfs, in the plaintext JSON
